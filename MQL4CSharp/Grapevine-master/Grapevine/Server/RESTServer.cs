    ...
    public class RESTServer : Responder, IDisposable
    {
        ...
        private void ProcessRequest(HttpListenerContext context)
        {
            ...
            try
            {
                ...
                //find all to match target
                var routes = this._routes.Where(mi => mi.GetCustomAttributes(true).Any(attr => rpath.Matches(((RESTRoute)attr).PathInfo) && context.Request.HttpMethod.ToUpper().Equals(((RESTRoute)attr).Method.ToString())));

                MethodInfo route = null;
                if (routes.Any())
                {
                    route = routes.OrderByDescending(o => o.Name.Length).First();
                }
