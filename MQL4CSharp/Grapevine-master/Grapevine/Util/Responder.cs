...
namespace Grapevine
{
    ...
    public abstract class Responder
    {
        ...
        {
            byte[] bytes = context.Request.ContentEncoding.GetBytes(json.ToString(Formatting.Indented, new JsonConverter[0]));
            int length = bytes.Length;
            context.Response.ContentType = ContentType.JSON.ToValue();
            context.Response.ContentEncoding = Encoding.UTF8;
            context.Response.ContentLength64 = length;
            context.Response.OutputStream.Write(bytes, 0, length);
        ...
        protected JObject GetJsonPayload(HttpListenerRequest request)
        {
            try
            {
                ...
                if (!object.ReferenceEquals(payload, null))
                {
        ...
        protected void SendJsonResponse(HttpListenerContext context, object obj)
        {
            ...
            byte[] bytes = context.Request.ContentEncoding.GetBytes(s);
            int length = bytes.Length;
            context.Response.ContentType = ContentType.JSON.ToValue();
            context.Response.ContentEncoding = Encoding.UTF8;
            context.Response.ContentLength64 = length;
